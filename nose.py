# -*- coding: utf-8 -*-
"""
Created on Thu May 23 21:16:10 2019
...
for i in range(1,21):     
    ...
    T = threshold_local(V, 79, offset=15, method="gaussian")
    thresh6 = (V > T).astype("uint8") * 255
    ...
    thresh = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 125, 8)
    
    ret3, th3 = cv2.threshold(blur, 15, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)

    
    imageContours, contours, hierarchy = cv2.findContours(th3, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
